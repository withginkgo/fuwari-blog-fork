...
## 五、git restore
以下是 `git restore` 和 `git commit --amend` 的中文详细对比说明。

---

### `git restore`

**作用：** 丢弃工作区或暂存区中的本地修改。用于**撤销未提交**的改动。
...
**重要特点：** `git restore` 只操作**未提交的修改**，不会改动任何已有的提交记录。

---

### `git commit --amend`

**作用：** 修改**最近一次提交**。可以用来更改提交信息、补充遗漏的文件、或者删除多余的文件。

**常见用法：**
- 修改最后一次提交的信息：  
  `git commit --amend -m "新的提交信息"`
- 将暂存区的改动合并到上一次提交，不修改信息：  
  `git add 遗漏的文件`  
  `git commit --amend --no-edit`
- 同时补充改动并修改提交信息：  
  `git add .`  
...
**重要特点：** `git commit --amend` 会**重写历史**。如果最后一次提交已经推送到远程仓库，你需要强制推送（`git push --force`），这可能会给协作伙伴带来麻烦。

---

### 主要区别对比表

| 对比项 | `git restore` | `git commit --amend` |
|--------|---------------|----------------------|
| 作用对象 | 未提交的改动（工作区 / 暂存区） | 最近一次提交 |
| 是否修改已有提交 | 否 | 是（重写历史） |
| 典型用途 | 撤销本地编辑、将文件移出暂存区 | 修正最后一次提交的信息或内容 |
| 安全性 | 非常安全，只影响本地未提交的工作 | 若提交已推送，则需谨慎（需强制推送） |
...
#### 使用 `git restore`
你编辑了一个文件，但后悔了，想恢复到上次提交时的状态：  
`git restore myfile.js` → 文件恢复原样。

#### 使用 `git commit --amend`
你提交时写错了信息：  
`git commit -m "修复 bug"` → 哦，应该是“修复 bug #42”  
`git commit --amend -m "修复 bug #42"` → 提交信息被修正。

或者你忘记把某个文件加入上一次提交：  
`git add 忘记的文件.js`  
`git commit --amend --no-edit` → 上一次提交现在包含这个文件了。

---

### 总结

- **`git restore`**：用于在**提交之前**撤销本地改动。
- **`git commit --amend`**：用于在**提交之后、推送之前**修正最近一次提交。

⚠️ 如果最后一次提交已经推送到**共享分支**，请勿使用 `amend`，除非你和团队协商好并强制推送。
